...
| notes | text | ◯ | | メモ |
| coverage_scope | smallint | ✕ | INDEX / default: 0 / `0 : all_japan` `1 : prefectures` `2 : cities` | 対応地域単位 |
...
### ● part_snapshots ― 部品スナップショット

| 列名 | 型 | NULL | 制約・インデックス | 説明 |
| -- | -- | -- | -- | -- |
| **id** | bigint (AI) | ✕ | **PK** | |
| **part_id** | bigint | ✕ | FK parts(id) / INDEX | |
| **checksum** | varchar | ✕ | INDEX | |
| material_category_code | char(10) | ✕ | FK→`m_categories.code` | 材質カテゴリ（WOOD / METAL …） |
| material_code | char(16) | ✕ | FK→`m_materials.code` | 材質（樹種など） |
| shape_code | char(8) | ✕ | FK→`m_shapes.code` | 平面形状（四角形など） |
| paint_type_code | char(4) | ◯ | FK→`m_paint_types.code` | 塗装種類 (選択なし・ウレタン・自然塗装…) |
...
### ● recipe_snapshot_parts ― 中間テーブル

|       Column       |              Type              | Collation | Nullable |                      Default|
|--------------------|--------------------------------|-----------|----------|---------------------------------------------------|
| id                 | bigint                         |           | not null | nextval('recipe_snapshot_parts_id_seq'::regclass)|
...
* **Indexes:**  
    "recipe_snapshot_parts_pkey" PRIMARY KEY, btree (id)  
    "index_recipe_snapshot_parts_on_part_snapshot_id" btree (part_snapshot_id)  
    "index_recipe_snapshot_parts_on_recipe_snapshot_id" btree (recipe_snapshot_id)  
* **Foreign-key constraints:**  
    "fk_rails_0989e6ad85" FOREIGN KEY (recipe_snapshot_id) REFERENCES recipe_snapshots(id)  
    "fk_rails_eede368e91" FOREIGN KEY (part_snapshot_id) REFERENCES part_snapshots(id)  

### ● rfq_parts ― 中間テーブル

|      Column      |              Type              | Collation | Nullable |                Default|
|------------------|--------------------------------|-----------|----------|---------------------------------------|
| id               | bigint                         |           | not null | nextval('rfq_parts_id_seq'::regclass)|
...
* **Indexes:**  
    "rfq_parts_pkey" PRIMARY KEY, btree (id)  
    "index_rfq_parts_on_part_snapshot_id" btree (part_snapshot_id)  
    "index_rfq_parts_on_rfq_id" btree (rfq_id)  
* **Foreign-key constraints:**  
    "fk_rails_923102ee88" FOREIGN KEY (part_snapshot_id) REFERENCES part_snapshots(id)  
    "fk_rails_dea4f7c049" FOREIGN KEY (rfq_id) REFERENCES rfqs(id)  

### ● vendor_offers — 加工業者別見積ヘッダ

|    Column     |              Type              | Collation | Nullable |                  Default|
|---------------|--------------------------------|-----------|----------|-------------------------------------------|
| id            | bigint                         |           | not null | nextval('vendor_offers_id_seq'::regclass)|
| rfq_id        | bigint                         |           | not null ||
...
| created_at    | timestamp(6) without time zone |           | not null ||
| updated_at    | timestamp(6) without time zone |           | not null ||
| sub_total     | numeric(18,4)                  |           | not null | 0.0|
| shipping_fee  | numeric(18,4)                  |           | not null | 0.0|
| tax_rate      | numeric(4,2)                   |           | not null | 10.0|
| tax_amount    | numeric(18,4)                  |           | not null | 0.0|
| received_at   | timestamp(6) without time zone |           |          ||
| created_by_id | bigint                         |           |          ||
| updated_by_id | bigint                         |           |          ||
| deleted_flag  | boolean                        |           | not null | false|
| deleted_at    | timestamp(6) without time zone |           |          ||
| deleted_by_id | bigint                         |           |          ||

* **Indexes:**  
    "vendor_offers_pkey" PRIMARY KEY, btree (id)  
    "index_vendor_offers_on_created_by_id" btree (created_by_id)  
    "index_vendor_offers_on_deleted_by_id" btree (deleted_by_id)  
    "index_vendor_offers_on_rfq_id" btree (rfq_id)  
    "index_vendor_offers_on_rfq_id_and_status" btree (rfq_id, status)  
    "index_vendor_offers_on_status" btree (status)  
    "index_vendor_offers_on_updated_by_id" btree (updated_by_id)  
    "index_vendor_offers_on_vendor_id" btree (vendor_id)  
    "index_vendor_offers_on_vendor_id_and_status" btree (vendor_id, status)  
    "uniq_vendor_offer_per_rfq" UNIQUE, btree (rfq_id, vendor_id) WHERE deleted_flag = false  
* **Foreign-key constraints:**  
    "fk_rails_37424dfe2f" FOREIGN KEY (updated_by_id) REFERENCES users(id)  
    "fk_rails_4fe5613415" FOREIGN KEY (deleted_by_id) REFERENCES users(id)  
    "fk_rails_8ad36f27c5" FOREIGN KEY (created_by_id) REFERENCES users(id)  
    "fk_rails_a3eca65d17" FOREIGN KEY (rfq_id) REFERENCES rfqs(id)  
    "fk_rails_b8710e984c" FOREIGN KEY (vendor_id) REFERENCES users(id)  

`¹ UNIQUE(quote_id, vendor_id)  -- 同一業者の重複見積を禁止`

### ● vendor_offer_items — 加工業者別見積明細

| 列名 | 型 | NULL許可 | 制約 | 説明 |
| -- | -- | -- | -- | -- |
| id | bigint | ✕ | **PK / AUTO_INCREMENT** | |
| vendor_offer_id | bigint | ✕ | FK → `vendor_offers.id` / **UNIQUE¹** / INDEX | 紐づく送付状 |
| rfq_part_id | bigint | ✕ | FK → `rfq_parts.id` / **UNIQUE¹** / INDEX | 会員側ベース明細を参照 |
...
* **自動計算をモデルで行う例**  
   ...
   class VendorOffer < ApplicationRecord
     ...
     private
     def calc_totals
       self.sub_total   = vendor_offer_items.sum("unit_price * quantity")
       self.total_price = sub_total + shipping_fee
       self.tax_amount  = (total_price * tax_rate / 100).round(2)
     end
   end
   ```
...
| **vendor_offer_id** | bigint | ✕ | FK → `vendor_offers.id` | 元となる見積 |
...
| **shipping_address_id** | bigint | ✕ | FK→`transaction_shipping_addresses.id` <br>ON UPDATE CASCADE / ON DELETE RESTRICT <br>INDEX | 依頼時点で選択した送付先 |
...
将来必要になった際は `order_items` を追加し、`vendor_offer_items` の明細情報をバックフィルすれば安全に拡張可能です。  
* 金額・数量に関する詳細はすべて採用した `vendor_offers`／`vendor_offer_items` を JOIN して参照できます。

## 取引系 (外部連携)

### ● stripe_accounts — Express Connected Account の ID
...
* **まとめ**
  ...
  | **サイズ監視** | `pg_total_relation_size('h_audit_trails') / pg_size_pretty()` を週次で Datadog 送信 |
...
### ● h_payout_events ─ 振込レコード (`payouts`) の内部ライフサイクルイベント

| 列名 | 型 | NULL許可 | 制約・インデックス | 説明 |
| -- | -- | -- | -- | -- |
| **id** | bigint | ✕ | **PK** / AUTO_INCREMENT | |
| **payout_id** | bigint | ✕ | FK → payouts.id / INDEX | 関連する振込ヘッダ |
...
* 将来パーティションに移す場合は `ALTER TABLE … PARTITION BY RANGE (occurred_at)` ＋ `CREATE TABLE … PARTITION OF …` でオンライン移行できます  
 実データ 50 万行/年以上になったら移行を検討する  
* **h_payout_events が “わざわざ別テーブル” で存在する理由**  
     | 項目                    | 役割・メリット                                                                                                                                             | 具体例                                                                                                                                                                                                                                     |
     | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
     | **状態遷移の履歴化**          | `payouts` は *今この瞬間のステータス* を 1 行で保持します。<br>しかし実務では「いつキューに載せたか」「API 送信は何時か」「失敗したので何回リトライしたか」を追跡したいケースが多い。<br>イベントを別テーブルに積み上げると、この**タイムライン**をロスなく残せます。 | <ul><li>2025-05-10 09:00 `queued`</li><li>2025-05-10 09:05 `send_api`</li><li>2025-05-10 09:07 `failed` <small>（meta → {error:"Insufficient funds"}）</small></li><li>2025-05-10 10:15 `retry`</li><li>2025-05-10 10:17 `paid`</li></ul> |
     | **デバッグ & 障害調査**       | 「なぜ `status=failed` になったのか？」を **メタ情報付きで即検索**できます。Slack アラートや Datadog ダッシュボードに貼る URL は `/payouts/123/events` だけで済む。                                  | `SELECT * FROM h_payout_events WHERE payout_id = 123 ORDER BY occurred_at;`                                                                                                                                                             |
     | **リトライ & 冪等制御**       | 決済／銀行 API は **途中で 500 が返る → 再送** ということが頻繁に起きる。<br>同じ `event='send_api'` を複数回入れるだけで「リトライ回数」や「最後の成功／失敗」が簡単に集計できます。                                    | `SELECT COUNT(*) FROM h_payout_events WHERE payout_id=123 AND event='send_api';`                                                                                                                                                        |
     | **監査証跡 (J-SOX/GDPR)** | 失敗を人手で再実行・金額を補正した場合など、**操作ログ**（`logged_by_type='Admin'`）を残すことで<br>*誰が／いつ／何をしたか* が明確になります。                                                           |                                                                                                                                                                                                                                         |
     | **BI / SLA 分析**       | 「振込キュー投入から実際に `paid` になるまでの平均処理時間」「月次の失敗率」などを BigQuery や Metabase で直接集計可能。<br>状態を 1 行で上書きする方式より可観測性が高い。                                             | `avg(paid.occurred_at - queued.occurred_at)` など（CTE で自己 JOIN）                                                                                                                                                                           |
     | **安全に拡張できる**          | 将来 “手数料計算を再実行した” といった新イベントが増えても **列追加は不要**。`event='fee_recalc'` で差分データを `meta_json` に載せるだけ。                                                         |                                                                                                                                                                                                                                         |
* **運用イメージ**  
   1. **業務サービス／ジョブ**  
      ```ruby
      # 例: Stripe Transfer 成功時
      HPayoutEvent.create!(
        payout_id:      payout.id,
        event:          'paid',
...
| latitude | smallint | ◯ | | 緯度 |
| longitude | smallint | ◯ | | 経度 |
| **created_at** | timestamptz | ✕ | DEFAULT CURRENT_TIMESTAMP | レコード作成日時 |
| **created_by_id** | bigint | ◯ | FK → users.id | レコード作成者 |
| **updated_at** | timestamptz | ✕ | DEFAULT CURRENT_TIMESTAMP | 最終更新日時 |
| **updated_by_id** | bigint | ◯ | FK → users.id | 最終更新者 |
...
### ● m_postal_codes ― 郵便番号マスタ（日本郵便 準拠）

| 列名 | 型 | NULL許可 | 制約・インデックス | 説明 |
| -- | -- | -- | -- | -- |
| **id** | bigint | ✕ | **PK** / AUTO_INCREMENT | 郵便番号マスタ主キー |
| **postal_code** | char(7) | ✕ | INDEX / CHECK(char_length(postal_code)=7) | 郵便番号 |
| city_code | char(5) | ✕ | FK→`m_cities.code` / INDEX | 全国地方公共団体コード |
| city_town_name_kanji | text | ✕ | | 市区町村名漢字 |
| town_area_name_kanji | text | ◯ | | 町域名漢字 |
| multi_town_flag | boolean | ✕ | DEFAULT false | 一町域が二以上の郵便番号を持つ場合の表示 |
| koaza_banchi_flag | boolean | ✕ | DEFAULT false | 小字毎に番地が起番されている町域の表示 |
| chome_flag | boolean | ✕ | DEFAULT false | 丁目を有する町域の場合の表示 |
| multi_aza_flag | boolean | ✕ | DEFAULT false | 一つの郵便番号で二以上の町域を表す場合の表示 |
| **created_at** | timestamptz | ✕ | DEFAULT CURRENT_TIMESTAMP | レコード作成日時 |
| **created_by_id** | bigint | ◯ | FK → users.id | レコード作成者 |
| **updated_at** | timestamptz | ✕ | DEFAULT CURRENT_TIMESTAMP | 最終更新日時 |
| **updated_by_id** | bigint | ◯ | FK → users.id | 最終更新者 |
...
#### メモ  
* *データは日本郵便 CSV「住所の郵便番号データ（utf_all.csv）」*
...
| name_en | varchar(20) | ✕ | UNIQUE | Wood, Metal … |
| **created_at** | timestamptz | ✕ | DEFAULT CURRENT_TIMESTAMP | レコード作成日時 |
| **created_by_id** | bigint | ◯ | FK → users.id | レコード作成者 |
| **updated_at** | timestamptz | ✕ | DEFAULT CURRENT_TIMESTAMP | 最終更新日時 |
| **updated_by_id** | bigint | ◯ | FK → users.id | 最終更新者 |
...
| category_code | char(10) | ✕ | FK → m_categories.code / INDEX | |
| name_ja | varchar(40) | ✕ | UNIQUE | |
| name_en | varchar(40) | ✕ | UNIQUE | |
...
| category_code | char(10) | ✕ | FK → m_categories.code / INDEX | 大分類 |
| name_ja | varchar(40) | ✕ | UNIQUE | アカマツ, A5052 … |
| name_en | varchar(40) | ✕ | UNIQUE | Red Pine, Aluminum 5052 |
| description\_ja | varchar(80) | ◯ | | 詳細説明 |
| description\_en | varchar(80) | ◯ | | 詳細説明 |
...
* **ユースケース**  
   ...
   `quote_items` 側では **コード値のみ** を保持し、表示時に `m_corner_processes` へ JOIN してラベルを得る。  
...
* **使い方例**  
   ```ruby
   # 例: 角R5 のマスタ行を取得
   MCornerProcess.find('ROUND')

   # 例: 直径6 mm の穴をラベル付きで表示
   hole = MHoleDiameter.find('6mm')
   puts "#{hole.hole_mm}mm (#{hole.name_ja})"
   ```